...
import com.example.weatherexpect.Tool.ActivityRequest.POLL_INTERVAL
...
class PollService: Service()
{
        ...
        fun setServiceAlarm(context: Context, isOn: Boolean) {
            ...
            if (isOn) {
                alarmManager.setInexactRepeating(
                    AlarmManager.ELAPSED_REALTIME_WAKEUP,       // ← 改：加 _WAKEUP，屏幕灭也触发
                    SystemClock.elapsedRealtime() + 60_000,     // ← 改：1分钟后第一次触发（不是立刻）
                    POLL_INTERVAL_MS,                           // ← 改：30分钟间隔（不是1秒）
                    pi
                )
            } else {
    ...
    private suspend fun fetchWeatherData() {
        ...
        val location = "101010100" // 默认位置
        val cityName = "北京" // 默认城市
            ...
            when (result) {
                is DailyWeatherResult.Success -> {
                    if (result.daily.isNotEmpty()) {
                        sendWeatherNotification(result.daily[0])
                    }else{
